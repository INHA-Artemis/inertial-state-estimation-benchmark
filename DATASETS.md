...
현재 공통 실행기에서 바로 읽을 수 있는 데이터는 EuRoC, i2Nav, Pohang입니다. CF231은 파일 구조와 학습 과정이 달라 `state-estimation-cf231-tcn` 명령으로 따로 실행합니다. UrbanNav는 과거 발표 결과만 남아 있고 현재 loader는 포함하지 않았습니다.
...
각 run 폴더에 IMU와 pose CSV가 모두 있어야 합니다. `poses.csv`는 학습 run의 속도 정답과 최종 평가에 사용합니다. 평가 대상인 Run 5에서는 초기 상태를 정한 뒤 IMU만 모델에 전달합니다.
