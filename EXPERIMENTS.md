...
이 문서는 저장소에 포함된 결과를 어떤 조건에서 만들었는지 기록한 것입니다. 결과를 비교할 때는 먼저 다음 세 경우를 구분해야 합니다.

- 현재 저장소의 코드로 다시 실행한 실험: 설정 파일과 결과 CSV 또는 JSON이 함께 남아 있습니다.
- 8월 4일 발표자료에서 가져온 수치: 당시 진행 내용을 확인하기 위해 보관했으며, 현재 코드의 최종 성능으로 사용하지 않습니다.
- 아직 다시 실행하지 못한 실험: 데이터나 설정이 충분하지 않아 비교에서 제외했습니다.
...
이 명령은 `reports/data/`에 저장된 수치로 EuRoC, 회전 운동, CF231 성능표와 8월 4일 비교 그림을 다시 만듭니다. `cf231_fixed_vs_tcn_trajectory.png`는 전체 trajectory 출력이 필요하므로 CF231 실행 결과에서 가져온 그림을 그대로 사용합니다.
